...
function toTwilioDate(date: Date): string {
  return date.toUTCString().replace('GMT', '+0000');
}

function buildInitialMessages(): TwilioMessage[] {
...
function toWhatsAppEvent(message: TwilioMessage): WhatsAppEventPayload {
  ...
  return {
    ...
    MediaResourceUrl__c:
      Number(message.num_media) > 0 ? message.subresource_uris?.media ?? null : null,
  };
}

const twilioMessages: TwilioMessage[] = buildInitialMessages();

export function getConversationByThreadId(threadId: string): ConversationResponse {
...
export function getMessagesByThreadId(_threadId: string): TwilioMessagesResponse {
...
export function publishMessage(input: PublishMessageInput): PublishMessageResult {
  ...
  const pusherEvent =
    input.direction === 'inbound' ? PUSHER_EVENT_INBOUND : PUSHER_EVENT_OUTBOUND;
